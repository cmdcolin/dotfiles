...
when storing the error in a react useState, store the actual error object and stringify it at point of use
...
in jbrowse-components, whenever we change a wgsl shader, we should use our compile-shader-utils to recompile it to glsl
...
Always commit with an explicit pathspec (`git commit -- <paths>`), never `git add` plus a bare `git commit`. Another agent may be working in the same worktree, and the git index is shared, so a bare commit silently sweeps up whatever they have staged (and theirs sweeps up yours)
